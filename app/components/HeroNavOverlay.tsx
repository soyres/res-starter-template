...
export default function HeroNavOverlay({
  ...
  return (
    <section className="relative min-h-dvh w-full overflow-hidden">
      ...
      {/* Panels - no animation on mount */}
      <div className="relative z-10 h-dvh w-full">
        <ul className={[
          "grid h-full grid-rows-3 divide-y divide-white/50",
          "md:grid md:grid-rows-1 md:grid-cols-3 md:divide-y-0 md:divide-x md:divide-white/50",
        ].join(" ")}>
          {links.map((l) => (
